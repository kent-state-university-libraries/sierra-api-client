 ...
 */  
    public function query($resource, $params = array(), $marc = false, $type = 'get') {
        ...
        $response = $this->_request($this->config['endpoint'] . $resource, $params, $headers, $type);
    ...
    private function _request($url, $params = array(), $header = array(), $type = 'get') {
        ...
        if ($type == 'post') {
            ...
            if (isset($params['body'])) {
                curl_setopt($s, CURLOPT_POSTFIELDS, $params['body']);
            }
            else {
                curl_setopt($s, CURLOPT_POSTFIELDS, http_build_query($params));
            }
